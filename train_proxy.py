...
class TrainingProxy:
    ...
    def build_train_command(self, stage: int, gop: int, 
        ...
        # I-frame模型参数
        cmd.extend(['--i_frame_type', self.config.get('i_frame_type', 'compressai')])
        if self.config.get('i_frame_type') == 'compressai':
            cmd.extend(['--i_frame_model_name', self.config.get('i_frame_model_name', 'cheng2020-anchor')])
            cmd.extend(['--i_frame_quality', str(self.config.get('i_frame_quality', 6))])
            if self.config.get('i_frame_pretrained', True):
                cmd.append('--i_frame_pretrained')
        elif self.config.get('i_frame_type') == 'vqgan':
...
def create_default_config(output_path: str):
    ...
    default_config = {
        ...
        # I-frame模型配置
        "i_frame_type": "compressai",  # "compressai" 或 "vqgan"
        "i_frame_model_name": "cheng2020-anchor",  # CompressAI模型名称
        "i_frame_quality": 6,  # CompressAI质量等级 (1-8)
        "i_frame_pretrained": True,  # 是否使用预训练权重
        
        # VQGAN配置（当i_frame_type="vqgan"时使用）
