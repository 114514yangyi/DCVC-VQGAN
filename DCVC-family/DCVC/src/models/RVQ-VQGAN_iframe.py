"""
RVQ-VQGAN I-frame 模型包装器

提供与 CompressAI I-frame 模型兼容的接口，使 RVQ-VQGAN 模型可以作为 I-frame 压缩模型使用。
"""
...
_VQGAN_ROOT = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '../../../vqgan/RVQ-VQGAN')
)
...
class VQGANIFrameModel(nn.Module):
    ...
    def _load_vqgan_model(self):
        """加载 RVQ-VQGAN taming 模型（直接使用原始 EMAVQ/VQModel 接口）"""
        ...
        # RVQ 层数（Residual Quantization 深度）
        ...
        # 加载 checkpoint（兼容 Lightning 保存格式）
    ...
    def forward(self, x):
        ...
        # 2. 编码和解码（直接使用 taming 模型的 encode/decode 接口）
        with torch.no_grad():
            quant, emb_loss, info = self.vqgan_model.encode(x_normalized)
            x_hat_normalized = self.vqgan_model.decode(quant)
