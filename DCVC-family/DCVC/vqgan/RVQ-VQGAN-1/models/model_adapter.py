...
class BaseVqVaeAdapter(nn.Module):
    ...
    def __init__(self, model: nn.Module):
        super().__init__()
        # 必须用 nn.Module 的 __setattr__ 注册子模块，否则 state_dict() 为空，
        # 会导致 checkpoint 中不包含真实模型权重，从而无法 resume。
        self._model = model
...
class TamingVQGANAdapter(BaseVqVaeAdapter):
    ...
    def __init__(self, model_args: Dict[str, Any], config_path: str):
        from models.taming.models.vqgan import EMAVQ, VQModel
        ...
        # RVQ 量化层数（Residual Quantization 深度），默认为 1 即退化为普通 EMA VQ
        num_quantizers = int(merged.pop("rvq_levels", merged.pop("num_quantizers", 1)))
