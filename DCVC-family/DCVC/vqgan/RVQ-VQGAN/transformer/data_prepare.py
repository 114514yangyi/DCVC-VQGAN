...
def encode_video_to_indices(
    ...
    输出:
        - 普通 VQ: (num_frames, h, w) int
        - 残差 RVQ: (num_frames, D, h, w) int
    """
...
def process_videos_to_indices(
    ...
    for video_path in tqdm(video_files, desc="训练集编码"):
        try:
            ...
            # 支持单层 VQ (D=1) 与多层 RVQ (D>1)
            if indices.ndim == 3:
                num_frames, ih, iw = indices.shape
                d = 1
            elif indices.ndim == 4:
                num_frames, d, ih, iw = indices.shape
            else:
                raise ValueError(f"不支持的 index 维度: {indices.shape}")

            indices_per_frame = d * ih * iw
...
def process_val_videos_to_indices_and_recon(
    ...
    for video_path in tqdm(video_files, desc="验证集编码与重构"):
        try:
            ...
            with torch.no_grad():
                # 残差 RVQ: indices_recon 形状可能为 [D, H, W] 或 [D, H, W] 堆叠在 batch 维
