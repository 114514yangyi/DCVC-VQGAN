...
def _get_indices_from_encode(vqgan_model, x: torch.Tensor, device: str) -> torch.Tensor:
    """调用 VQGAN 内部 encode，返回 codebook indices。

    标准 VQ：
        - 返回形状 (B, H, W)
    PQ / 其他多子空间量化：
        - 返回形状 (B, S, H, W)，S 为子空间数
    """
...
def encode_video_to_indices(
    ...
    输入：
        video: (num_frames, H, W, C) uint8

    输出：
        - 标准 VQ:    (num_frames, h, w) int
        - PQ / 多子空间: (num_frames, S, h, w) int
    """
...
def decode_indices_to_video(
    ...
    支持：
        ...
        - indices: (num_frames, S, h, w)

    返回：
        (num_frames, H, W, C) uint8。
    """
    ...
    with torch.no_grad():
        for i in range(0, num_frames, batch_size):
            ...
            # 对于 PQ，embed_code 期望形状为 (B, S, h, w)；标准 VQ 则为 (B, h, w)
...
def process_videos_to_indices(
    ...
    标准 VQ：
        - indices 形状 (num_frames, h, w)
        - 每帧 index 数为 h * w

    PQ / 多子空间：
        - indices 形状 (num_frames, S, h, w)
        - 每帧 index 数为 S * h * w

    返回：
        data: [num_samples, seq_len]，其中
              seq_len = sequence_length * h * w      (VQ)
                      = sequence_length * S * h * w  (PQ)
    """
    ...
    for video_path in tqdm(video_files, desc="训练集编码"):
        try:
            ...
            if indices.dim() == 3:
                ...
                num_frames, num_subspaces, ih, iw = indices.shape
            else:
                raise ValueError(f\"不支持的 indices 维度: {indices.shape}\")

            indices_per_frame = ih * iw * num_subspaces
            num_segments = num_frames // sequence_length
            for i in range(num_segments):
                start = i * sequence_length
                end = start + sequence_length
                # 将时间 + 空间 + 子空间展平成一维序列
                seg = indices[start:end].reshape(-1).cpu().numpy().astype(np.int64)
