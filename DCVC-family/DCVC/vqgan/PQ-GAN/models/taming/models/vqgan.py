...
try:
    ...
    from models.taming.modules.vqvae.quantize import EMAVectorQuantizer, PQEMAVectorQuantizer
except ImportError:
    ...
    try:
        ...
        from taming.modules.vqvae.quantize import EMAVectorQuantizer, PQEMAVectorQuantizer
    except ImportError:
...
class PQEMAVQ(VQModel):
    """
    使用 Product Quantization + EMA 码本更新的 VQGAN 变体。

    与 EMAVQ 的区别：
    - 量化器从单一 EMA 码本改为 S 个子空间独立 EMA 码本（PQEMAVectorQuantizer）；
    - 其余编码器 / 解码器 / 损失结构保持不变，方便与 VQ 基线公平对比。
    """
    ...
    def __init__(
        ...
        # 先用父类构建编码器 / 解码器 / loss 等
        super().__init__(
            ddconfig=ddconfig,
            lossconfig=lossconfig,
            n_embed=n_embed,
            embed_dim=embed_dim,
            ckpt_path=None,  # 先不加载 ckpt，避免覆盖量化器
            ignore_keys=ignore_keys,
            image_key=image_key,
            colorize_nlabels=colorize_nlabels,
            monitor=monitor,
        ...
        # 替换量化器为 PQ + EMA
        self.quantize = PQEMAVectorQuantizer(
            n_embed=n_embed,
            embedding_dim=embed_dim,
            beta=0.25,
        ...
        # 如有 ckpt，再加载其余权重（除量化器外）
        if ckpt_path is not None:
            self.init_from_ckpt(ckpt_path, ignore_keys=ignore_keys)

    def configure_optimizers(self):
        """
        与 EMAVQ 一致：量化器参数不进入优化器，仅对 encoder/decoder/conv 做梯度更新。
        """
        lr = self.learning_rate
