...
def process_videos_to_indices(
    ...
    """
    从视频目录加载视频，使用 VQGAN 编码，然后按照 sequence_length 做“滑动窗口”（步长=1）切分为训练样本
    ...
    for video_idx, video_path in tqdm(enumerate(video_files)):
        try:
            ...
            # 按照 sequence_length 做滑动窗口切分（步长=1）
            # 只处理完整的窗口；不足 sequence_length 的视频会被跳过
            if num_frames < sequence_length:
                continue
            num_segments = num_frames - sequence_length + 1
