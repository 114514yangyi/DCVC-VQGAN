...
def train_epoch(model, dataloader, criterion, optimizer, device, epoch,
    ...
    for batch_idx, indices in enumerate(pbar):
        ...
        with torch.amp.autocast(device_type='cuda', enabled=use_amp):
            ...
            logits_flat = logits.reshape(-1, logits.size(-1))  # [batch * seq_len, num_codes]
            ...
            accuracy = (preds == target_seq).float().mean()
...
def test_sample(args, device, train_loader, model, best_model_path):
    """
    测试样本：使用训练好的模型进行逐步预测和一次性预测
    ...
    # 创建一个测试样本（取第一个 batch 的第一个样本）
    
    fixed_bits = np.log2(args.vocab_size)
    test_sample = next(iter(train_loader)).to(device)  # DataLoader 返回的是 [batch_size, seq_len]
    test_sample = test_sample[0:1]  # 只取第一个样本，保持 batch 维度 [1, seq_len]
    
    # 模型加载最优的模型
    model.load_state_dict(torch.load(best_model_path, map_location=device, weights_only=False)['model_state_dict'])
    
    # ========== 逐步预测测试：使用前256个index作为前置信息，逐个预测下一个index ==========
    print(f"\n{'='*60}")
    print("逐步预测测试：使用前256个index作为前置信息")
    print('='*60)
    
    prefix_length = 256
    seq_len = args.seq_len
    test_indices = test_sample[0]  # (seq_len,)
    
    if seq_len <= prefix_length:
        print(f"⚠ 序列长度 ({seq_len}) 小于等于前缀长度 ({prefix_length})，跳过逐步预测测试")
    else:
        # 提取前256个index作为前置信息
        prefix_indices = test_indices[:prefix_length].clone()  # (prefix_length,)
        remaining_indices = test_indices[prefix_length:]  # (remaining_length,)
        remaining_length = len(remaining_indices)
        ...
        # 初始化 context 应该包含 BOS + 前缀
        current_context = torch.cat([bos_tensor, prefix_indices], dim=0)
        
    with torch.no_grad():
        for pos in tqdm(range(remaining_length), desc=f"逐步预测:{len(all_correct)}"):
            # 使用Transformer的get_conditional_probs方法获取下一个位置的条件概率
            # current_context: (context_len,)
            if current_context[0] != bos_id:
                raise ValueError(f"current_context[0] != bos_id: {current_context[0]} != {bos_id}")
            next_probs = model.transformer.get_conditional_probs(current_context, temperature=1.0, use_rope=True)
            # next_probs: (num_codes,)
            
            # 预测下一个index（使用argmax）
            pred_idx = next_probs.argmax().item()
            
            # 获取真实index
            true_idx = remaining_indices[pos].item()
            
            # 获取真实index的概率
            true_prob = next_probs[true_idx].item()
            ...
            # 打印前10个和每1000个位置的预测结果
            if pos < 10 or pos % 1000 == 0 or not is_correct:
                status = "✓" if is_correct else "✗"
                print(f"  {status} 位置 {pos+prefix_length} (剩余位置 {pos}): 真实={true_idx}, 预测={pred_idx}, 真实概率={true_prob:.6f}")
            
            # 将真实index添加到上下文（用于下一步预测）
            # 注意：这里使用真实index，而不是预测的index，以模拟压缩时的场景
            current_context = torch.cat([current_context, torch.tensor([true_idx], dtype=torch.long, device=device)], dim=0)
        
        # 统计结果
        all_predictions = torch.tensor(all_predictions, device=device)
        all_correct = torch.tensor(all_correct, device=device)
        accuracy = all_correct.float().mean().item()
        num_correct = all_correct.sum().item()
        num_total = len(all_correct)
        
        print(f"\n逐步预测结果统计:")
        print(f"  总预测数: {num_total}")
        print(f"  正确预测数: {num_correct}")
        print(f"  错误预测数: {num_total - num_correct}")
        print(f"  准确率: {accuracy:.4f} ({accuracy*100:.2f}%)")
        
        # 显示错误预测的详细信息
        # if num_correct < num_total:
        #     print(f"\n错误预测详情（前20个）:")
        #     print("位置 | 真实索引 | 预测索引 | 真实概率")
        #     print("-" * 50)
        #     error_count = 0
        #     for pos in range(remaining_length):
        #         if not all_correct[pos]:
        #             true_idx = remaining_indices[pos].item()
        #             pred_idx = all_predictions[pos].item()
        #             # 重新计算这个位置的预测概率（用于显示）
        #             bos_id = model.transformer.bos_token_id
        #             bos_tensor = torch.tensor([bos_id], dtype=torch.long, device=device)
        #             context_temp = torch.cat([bos_tensor, prefix_indices, remaining_indices[:pos]], dim=0)
        #             probs_temp = model.transformer.get_conditional_probs(context_temp, temperature=1.0)
        #             true_prob = probs_temp[true_idx].item()
        #             print(f"{pos+prefix_length:4d} | {true_idx:8d} | {pred_idx:8d} | {true_prob:.6f}")
        #             error_count += 1
        #             if error_count >= 20:
        #                 break
    ...
    with torch.no_grad():
        # 添加BOS token到测试样本
        bos_token_id = model.transformer.bos_token_id
        bos_column = torch.full((test_sample.size(0), 1), bos_token_id, dtype=torch.long, device=device)
        test_sample_with_bos = torch.cat([bos_column, test_sample], dim=1)  # [batch, seq_len+1]
        
        # 生成因果掩码
        seq_len_with_bos = test_sample_with_bos.size(1)
        mask = torch.triu(torch.ones(seq_len_with_bos, seq_len_with_bos), diagonal=1).to(device)
        
        # 前向传播
        logits, probs = model.transformer(test_sample_with_bos, mask=mask, use_rope=True)
        # logits, probs: [batch, seq_len+1, num_codes]
        
        # 计算率损失
        input_logits = logits[:, :-1, :].contiguous()  # [batch, seq_len, num_codes]
        target_indices = test_sample_with_bos[:, 1:].contiguous()  # [batch, seq_len]
        logits_flat = input_logits.view(-1, logits.size(-1))
        indices_flat = target_indices.view(-1)
        nll_loss = F.cross_entropy(logits_flat, indices_flat, reduction='mean')
        rate = nll_loss / 0.6931471805599453
        
        # 预测结果（只比较非BOS位置）
        # probs[:, :-1, :] 对应预测 test_sample_with_bos[:, 1:]（即 test_sample）
        preds = torch.argmax(probs[:, :-1, :], dim=-1)  # [batch, seq_len]
        accuracy = (preds == test_sample).float().mean()
        
        # 计算估计比特数
        estimated_bits = rate.item() * args.seq_len
        
        print(f"测试样本准确率: {accuracy.item():.4f}")
        print(f"估计比特数: {estimated_bits:.2f} bits")
        print(f"原始比特数: {args.seq_len * fixed_bits:.2f} bits (固定长度)")
        print(f"节省比特数: {args.seq_len * fixed_bits - estimated_bits:.2f} bits")
        
        # 显示前10个位置的预测
        print("\n前10个位置预测:")
        print("位置 | 真实索引 | 预测索引 | 概率")
        print("-" * 40)
        for i in range(min(10, args.seq_len)):
            true_idx = test_sample[0, i].item()
            pred_idx = preds[0, i].item()
            # probs[:, :-1, :] 对应 test_sample，所以 probs[0, i, :] 对应位置i
            prob = probs[0, i, true_idx].item()
            match = "✓" if true_idx == pred_idx else "✗"
            print(f"{i:4d} | {true_idx:8d} | {pred_idx:8d} | {prob:.6f} {match}")

# def test_sample():
#     parser = argparse.ArgumentParser(description='训练 Transformer 压缩 VQ-VAE 索引')
#     parser.add_argument('--vocab_size', type=int, default=1024, help='codebook 大小')
#     parser.add_argument('--seq_len', type=int, default=16384, help='序列长度 (16x16=256)')
#     parser.add_argument('--batch_size', type=int, default=4, help='批大小')
#     parser.add_argument('--epochs', type=int, default=5, help='训练轮数')
#     parser.add_argument('--lr', type=float, default=1e-4, help='学习率')
#     parser.add_argument('--d_model', type=int, default=256, help='Transformer 隐藏维度')
...
#     best_model_path="/home/huyang/VqVaeVideo-master/VqVaeVideo-master/transformer/outputs/best_model.pth"
#     fixed_bits=np.log2(args.vocab_size)
#     test_sample = next(iter(train_loader)).to(device)  # DataLoader 返回的是 [batch_size, seq_len]
#     test_sample = test_sample[0:1]  # 只取第一个样本，保持 batch 维度 [1, seq_len]
...
#         all_predictions = []
#         all_correct = []
#         current_context = prefix_indices.clone()  # 当前上下文
        
#         with torch.no_grad():
#             for pos in tqdm(range(remaining_length), desc="逐步预测"):
#                 # 使用Transformer的get_conditional_probs方法获取下一个位置的条件概率
#                 # current_context: (context_len,)
#                 next_probs = model.transformer.get_conditional_probs(current_context, temperature=1.0)
#                 # next_probs: (vocab_size,)
...
#                     pred_idx = all_predictions[pos].item()
#                     # 重新计算这个位置的预测概率（用于显示）
#                     context_temp = torch.cat([prefix_indices, remaining_indices[:pos]], dim=0)
#                     probs_temp = model.transformer.get_conditional_probs(context_temp, temperature=1.0)
#                     true_prob = probs_temp[true_idx].item()
#                     print(f"{pos+prefix_length:4d} | {true_idx:8d} | {pred_idx:8d} | {true_prob:.6f}")
#                     error_count += 1
...
#     model.eval()
#     with torch.no_grad():
#         rate, probs = model(test_sample, return_probs=True)
