...
# CompressAI 优化器工具（仅用于 DCVC P-frame 模型）
...
def compress_i_frame_with_padding(i_frame_model, frame_tensor, calculate_bpp=True, i_frame_size_bytes=None):
    """
    ...
    if calculate_bpp:
        num_pixels = B * H * W
        # 如果显式提供了 I-frame 大小（字节），优先使用该参数计算 BPP
        if i_frame_size_bytes is not None:
            total_bits = float(i_frame_size_bytes) * 8.0
            bpp = total_bits / num_pixels
            return encoded, bpp
        else:
            # 否则回退到基于 likelihoods 的精确 BPP 计算（CompressAI 兼容）
            total_bits = 0.0
            import math
...
def evaluate_video(model, i_frame_model, video_dir, device, stage, max_frames=96,
                   accelerator=None, i_frame_size_bytes=None):
    """
    在视频数据集上评估 DCVC 模型
    ...
    with torch.no_grad():
        ...
        for seq_idx in range(len(eval_dataset)):
            ...
            i_frame_result, i_frame_bpp = compress_i_frame_with_padding(
                ...
                i_frame_size_bytes=i_frame_size_bytes,
            )
...
def main():
    ...
    # Video dataset & RVQ-VQGAN I-frame arguments
    ...
    # 本脚本专用于 RVQ-VQGAN 作为 I-frame 模型，不再支持 CompressAI I-frame
    parser.add_argument('--vqgan_config', type=str, required=True,
                        help='RVQ-VQGAN config file path')
    parser.add_argument('--vqgan_checkpoint', type=str, required=True,
                        help='RVQ-VQGAN checkpoint path')
    parser.add_argument('--i_frame_size', type=float, default=None,
                        help='Fixed I-frame size in bytes for BPP calculation (if set, overrides model likelihoods)')
    ...
    # ========== 加载 I-frame 模型（RVQ-VQGAN） ==========
    # 本脚本专门使用 RVQ-VQGAN 作为 I-frame 模型
    if accelerator.is_main_process:
        logger.info("Loading RVQ-VQGAN I-frame model")
        logger.info(f"Config: {args.vqgan_config}")
        logger.info(f"Checkpoint: {args.vqgan_checkpoint}")

    try:
        # 动态加载 RVQ-VQGAN_iframe 包装器，避免文件名中的连字符问题
        import importlib.util
        rvq_iframe_path = os.path.join(os.path.dirname(__file__), "src/models/RVQ-VQGAN_iframe.py")
        spec = importlib.util.spec_from_file_location("rvq_vqgan_iframe", rvq_iframe_path)
        rvq_module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(rvq_module)
        VQGANIFrameModel = rvq_module.VQGANIFrameModel
        ...
        if accelerator.is_main_process:
            logger.info("Successfully loaded RVQ-VQGAN I-frame model")
            logger.info(f"Codebook size: {i_frame_model.codebook_size}")
    except Exception as e:
        error_msg = f"Error loading RVQ-VQGAN I-frame model: {e}"
        if accelerator.is_main_process:
    ...
    for epoch in range(start_epoch, config['epochs']):
        ...
        if (epoch + 1) % args.eval_freq == 0 and val_video_available:
            ...
            video_results = evaluate_video(
                ...
                config['eval_stage'], args.eval_max_frames,
                accelerator=accelerator, i_frame_size_bytes=args.i_frame_size
            )
